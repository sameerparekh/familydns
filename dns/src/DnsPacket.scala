...
object DnsPacket:
  ...
  def parseQuery(data: Array[Byte]): Option[Query] =
    ...
    try
      ...
      val domain = readDomain(buf, data)
  ...
  private def readDomain(buf: ByteBuffer, raw: Array[Byte]): String =
   ...
   * Sets QR=1, RCODE=3, copies question section, zeroes answer counts.
   */
  ...
  /**
   * Build a REFUSED response (for schedule/pause blocks — cleaner UX than NXDOMAIN
   * which gets cached by the client).
   */
