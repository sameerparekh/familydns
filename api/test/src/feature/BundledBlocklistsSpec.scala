...
object BundledBlocklistsSpec
  ...
  def spec = suite("BundledBlocklists")(
    ...
    test("V36 canonicalization: V1's Kids seed has canonical slugs after fresh migrate") {
      // V1__init.sql seeds Kids with ['adult','gambling','social_media','proxy'];
      // V36 rewrites 'social_media' -> 'social-media' and strips 'proxy'.
      for {
        _    <- cleanDb
        pr   <- ZIO.service[ProfileRepo]
        kids <- pr
    ...
    test("V36 canonicalization: idempotent — rewrites 'social_media' and strips 'proxy'") {
      // Inject a profile carrying the legacy slugs after migrate, then re-run the V36 SQL
    ...
    test("V36 canonicalization: leaves canonical slugs untouched") {
      import doobie.implicits.*
      for {
        _     <- cleanDb
        pr    <- ZIO.service[ProfileRepo]
