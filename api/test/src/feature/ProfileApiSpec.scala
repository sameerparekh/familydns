...
/** Feature tests for the Profile API.
  *
  * These exercise the full stack: HTTP handler → service → real Postgres.
  * No mocks except Clock and upstream DNS socket.
  */
object ProfileApiSpec extends ZIOSpec[TestDatabase.AllRepos & EmbeddedPostgres & Clock] {
  ...
  def spec = suite("Profile API")(
    ...
    suite("GET /api/profiles")(
      test("returns seeded default profiles") {
        for
          ...
          routes       = ProfileRoutes.routes(auth, profileRepo, schedRepo, tlRepo, stlRepo)
    ...
    suite("POST /api/profiles")(
      test("admin can create a profile with schedules and time limits") {
        for
          ...
          body         = UpsertProfileRequest(
                           ...
                           paused            = false,
                           schedules         = List(
                             ScheduleRequest("Bedtime",
                               List("mon","tue","wed","thu","fri"), "22:00", "08:00")
                           ),
                           timeLimit         = Some(180),
                           siteTimeLimits    = List(
          ...
          req          = Request.post(URL.decode("/api/profiles").toOption.get, Body.fromString(body))
                           .addHeader(Header.Authorization.render(s"Bearer $token"))
                           .addHeader(Header.ContentType(MediaType.application.json))
          resp        <- routes.runZIO(req)
          _           <- assertTrue(resp.status == Status.Ok).toZIO
          // Verify everything was persisted
          profiles    <- profileRepo.listAll
          teen        <- ZIO.fromOption(profiles.find(_.name == "Teenager"))
      ...
      test("readonly user cannot create profiles") {
        for
          _           <- cleanDb
          profileRepo <- ZIO.service[ProfileRepo]
          schedRepo   <- ZIO.service[ScheduleRepo]
          ...
          _           <- userRepo.create("reader", hash, "readonly")
    ...
    suite("POST /api/profiles/:id/pause")(
      test("toggles pause state") {
        ...
        yield
          assertTrue(resp1.status == Status.Ok) &&
