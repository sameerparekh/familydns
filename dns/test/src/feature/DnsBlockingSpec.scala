...
/** Feature tests for DNS blocking decisions using real DB state.
  *
  * These tests load profiles/devices/blocklists from an embedded Postgres,
  * build the cache exactly as the DNS server would, then run blocking decisions.
  * The controllable Clock lets us simulate bedtime, school hours, etc.
  */
object DnsBlockingSpec extends ZIOSpec[TestDatabase.AllRepos & EmbeddedPostgres & Clock] {
  ...
  private def buildCache: ZIO[
    ProfileRepo & ScheduleRepo & TimeLimitRepo & SiteTimeLimitRepo &
    DeviceRepo & BlocklistRepo, Throwable, DnsCache
  ] =
    for
  ...
  def spec = suite("DNS Blocking — full stack")(
    ...
    suite("category blocking loaded from DB")(
      test("blocks adult domain after loading blocklist from DB") {
        ...
        yield
          assertTrue(d1 == BlockingEngine.Decision.Block("category:adult")) &&
      ...
      test("adult device not blocked by kids blocklist") {
        ...
        yield
          assertTrue(kidDecision  == BlockingEngine.Decision.Block("category:social_media")) &&
    ...
    suite("unknown device")(
      test("unknown MAC falls through to default (adults) profile") {
        for
          ...
          d            = cache.deviceProfiles.get(unknownMac)
                           .map(p => BlockingEngine.decide("facebook.com", p, cache.blocklists,
                             TimeUsageSnapshot.empty, unknownMac, LocalTime.of(14,0), LocalDate.of(2025,1,6)))
                           .getOrElse(BlockingEngine.Decision.Allow)
        yield assertTrue(d == BlockingEngine.Decision.Allow)
      },
    ),
