...
/** Unit tests for BlockingEngine pure functions.
  *
  * These cover edge cases in blocking logic — schedule boundary conditions,
  * domain pattern matching edge cases, priority ordering.
  * No ZIO effects, no DB, no embedded Postgres.
  */
object BlockingEngineSpec extends ZIOSpecDefault {
  ...
  private val bedtimeSchedule = Schedule(
    id = 1L, profileId = 1L, name = "Bedtime",
  ...
  private val weekdayOnlySchedule = Schedule(
    id = 2L, profileId = 1L, name = "School",
  ...
  def spec = suite("BlockingEngine")(
    ...
    suite("schedule")(
      test("blocks during overnight schedule (21:00-07:00)") {
        ...
        assertTrue(decide("google.com", p, time = LocalTime.of(21, 30)) == Decision.Block("schedule")) &&
        assertTrue(decide("google.com", p, time = LocalTime.of(23, 59)) == Decision.Block("schedule")) &&
        assertTrue(decide("google.com", p, time = LocalTime.of(0, 0))   == Decision.Block("schedule")) &&
        assertTrue(decide("google.com", p, time = LocalTime.of(6, 59))  == Decision.Block("schedule"))
      },
      ...
      test("weekday-only schedule does not apply on Saturday") {
        ...
        assertTrue(decide("google.com", p, time = LocalTime.of(10, 0), date = saturday) == Decision.Allow)
      },

      test("weekday-only schedule applies on Monday") {
        val p = profile(schedules = List(weekdayOnlySchedule))
        assertTrue(decide("google.com", p, time = LocalTime.of(10, 0), date = monday) == Decision.Block("schedule"))
      },

      test("schedule boundary: exactly at block_from is blocked") {
        val p = profile(schedules = List(bedtimeSchedule))
        assertTrue(decide("google.com", p, time = LocalTime.of(21, 0)) == Decision.Block("schedule"))
      },
    ...
    suite("extra_allowed / extra_blocked")(
      ...
      test("extra_allowed does NOT override schedule") {
        ...
        assertTrue(decide("khanacademy.org", p, time = LocalTime.of(22, 0)) == Decision.Block("schedule"))
      },
    ),
    ...
    suite("time limits")(
      ...
      test("blocks when site-specific limit reached") {
        ...
        assertTrue(decide("youtube.com", p, usage = usage) == Decision.Block("site_time_limit:YouTube"))
      },
