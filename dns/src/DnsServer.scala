...
class DnsServer(
  ...
  def serve: Task[Unit] =
    ZIO.acquireReleaseWith(
      ...
      ZIO.loop(())(Function.const(true), identity) { _ =>
        for
    ...
    DnsPacket.parseQuery(data) match
      ...
      case Some(query) =>
        for
          ...
          profile  = mac.flatMap(m => cache.deviceProfiles.get(m))
          ...
          response <- profile match
            ...
            case Some(cp) =>
              ...
              val decision = BlockingEngine.decideWithBlocklists(
                query.domain, cp, cache.blocklists, usage,
                mac.getOrElse(""), now, today,
              )
              decision match
                ...
                case BlockingEngine.Decision.Block(reason) =>
                  val resp = if reason.startsWith("schedule") || reason == "paused"
                             then DnsPacket.buildRefused(data)
                             else DnsPacket.buildNxdomain(data)
                  ZIO.succeed(resp)
                      <* enqueueLog(mac, cp, query, blocked = true, reason, clientIp)
          _ <- ZIO.attempt:
  ...
  private def forwardUpstream(data: Array[Byte]): Task[Option[Array[Byte]]] =
    ZIO.attemptBlocking:
      upstreams.iterator.flatMap { (host, port) =>
        try
